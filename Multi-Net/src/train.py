...
def train_y():
    ...
    with tf.name_scope('input'):
        train_image_batch, train_label_batch = tfRecord.createShuffleBatch('../tfRecords/train/train.RTM.tfrecords', utils.batch_size)
        val_image_batch, val_label_batch = tfRecord.createShuffleBatch('../tfRecords/test/test.RTM.tfrecords', utils.eval_batch_size)
    x = tf.placeholder(tf.float32, shape=[None, utils.img_width, utils.img_height, utils.img_channels])
    y_ = tf.placeholder(tf.int16, shape = [None, utils.num_class])

    logits = vgg.vgg16('inputy',x, utils.num_class, utils.IS_PRETRAIN)
    loss = tools.loss(logits, y_)
    accuracy = tools.accuracy(logits, y_)
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter('../log/LogRTM', sess.graph)
        print('** sess.graph write sucessfully.')
            ...
            while True:
                ...
                if step <= utils.max_step:
                    ...
                    # if step%10 == 0 or (step+1) == utils.max_step:
                        ...
                        # print('** Time cost: {} **'.format(time_end-time_start))
                    if step%200 == 0 or step == utils.max_step:
                        checkpoint_path = '../model/TrainRTM' + '/model-RTM.ckpt'
                        saver.save(sess, checkpoint_path, global_step=step)
                else:
                    break
        except tf.errors.OutOfRangeError:
