...
def createBatch(filename, batch_size):
    ...
    imgRDM_batch, imgATM_batch, label_batch = tf.train.batch([imagesRDM, imagesATM, labels],
...
def test():
    ...
    img1_batch, img2_batch, label_batch = createBatch('../tfRecords/Test/test.tfrecords',61)
    with tf.Session() as sess:
        ...
        for i in range(2):
            example1, example2, l = sess.run([img1_batch, img2_batch, label_batch])
